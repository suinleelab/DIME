...
def HardAttentionTrainer(model, T, device, train_loader, val_loader, test_loader, nepochs, lr, tensorboard_file_name_suffix="logs", path="results", training_phase="first"):
        ...
        logger = SummaryWriter(log_dir=f"hard_attn_logs_phase_{training_phase}", filename_suffix=tensorboard_file_name_suffix)
        ...
        for epoch in range(nepochs):
            ...
            torch.save([model.state_dict(), optimizerG.state_dict()], path+'/weights_f_training_phase_{0}_imsz_224_lr_0.0001_detach_ccebal_1_auc.pth'.format(training_phase))
